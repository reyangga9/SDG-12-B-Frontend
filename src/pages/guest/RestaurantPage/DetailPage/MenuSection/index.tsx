          ...
          <div key={index} className="relative">
            <div className="card w-72 h-full bg-base-100 border hover:shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] hover:border-none mb-8 transition-all duration-300">
              <figure className="px-2 pt-2">
                <img
                  ...
                  className={`w-full h-60 border object-cover bg-gray-100 rounded-xl transition-all duration-500 ease-in-out ${!imageLoaded ? "blur-lg" : ""
                    } ${dayjs(food.tanggalExpired) < currentDate ? "filter grayscale" : ""}`}
                  onLoad={() => {
                    setTimeout(() => {
                      setImageLoaded(true);
                    }, 100);
              ...
              <div className="card-body px-3 py-3 ">
                ...
                {dayjs(food.tanggalExpired) < currentDate ? (
                ...
                <p className={`text-2xl font-semibold mt-2 ${dayjs(food.tanggalExpired) < currentDate ? 'text-red-500' : ''}`}>
                  {dayjs(food.tanggalExpired) < currentDate
                    ? "Out of Stock"
                    : new Intl.NumberFormat("id-ID", {
                      style: "currency",
                ...
                {foodCounts[food._id] > 0 ? (
                  ...
                  <button
                    className={`btn ${dayjs(food.tanggalExpired) < currentDate ? 'btn-base-300' : 'btn-primary'} normal-case text-base`}
                    onClick={() => {
                      if (isAuthenticated) {
                        handleIncrement(food._id, food.restoId);
                      } else {
                    ...
                    disabled={dayjs(food.tanggalExpired) < currentDate}
                  >
                    Add
                  </button>
                )}
